...
* **Скрыть выбором цели (трей/настройки).** Позволяет выбрать любое видимое окно перетаскиванием прицела и сразу скрыть его.
...
Скрытие выбором цели
------------------

1. Откройте `Settings...` в SmartSystemMenu.
2. На вкладке `General` нажмите `Hide...` в группе `Close Window By Mouse Click`.
3. Перетащите прицел на окно, которое нужно скрыть, и отпустите кнопку мыши.
4. Чтобы вернуть скрытые окна, используйте меню в трее `Restore` -> `Hide`.
